  ...
  */
  openDb() {
    ...
    this
      .db
      .transaction((tx) => {
        tx.executeSql('CREATE TABLE IF NOT EXISTS Todo (id integer primary key,imgid integer, image text, description text)');
      }, (e) => {
   ...
   */
  addItem(imgid,image, description) {
    return new Promise(resolve => {
      this.check(image).
        then((res)=>{
          console.log(res)
          if (res==false){
            var InsertQuery = "INSERT INTO Todo (imgid, image, description) VALUES (?,?,?)";
                  this
                    .db
                    .executeSql(InsertQuery, [imgid, image, description], (r) => {
        ...
        .executeSql(query, [], rs => {
          if (rs.rows.length > 0) {
            for (var i = 0; i < rs.rows.length; i++) {
                 this.arr.push({ imgid:rs.rows.item(i).imgid, image: rs.rows.item(i).image, description: rs.rows.item(i).description});
            }
          }
          res(true);
        }, (e) => {
  ...
  //to Update any Item
  update(imgid, image, description) {
    return new Promise(res => {
      var query = "UPDATE Todo SET description=?  WHERE image=?";
      this
        .db
        .executeSql(query, [
          imgid, description, image 
        ], (s) => {
